...
public class Auth extends AppCompatActivity {
    ...
    class C03971 implements Response.Listener<String> {
        ...
        public void onResponse(String response) {
           Auth.this.progress.dismiss();
            try {
                Auth.this.showJSON(response);
    ...
    class C03982 implements Response.ErrorListener {
        ...
        public void onErrorResponse(VolleyError error) {
          Auth.this.progress.dismiss();
            Toast.makeText(Auth.this, "Network connection error", Toast.LENGTH_LONG).show();
            finish();
        }
    }
...
if(response.contains("paid")) {
    final String gname = Auth.this.getIntent().getExtras().getString("gname");
    final String gid = Auth.this.getIntent().getExtras().getString("gid");
    Intent intent = new Intent(Auth.this, SMS.class);
...
else if(response.contains("pay"))
{
    final String gname = Auth.this.getIntent().getExtras().getString("gname");
    final String gid = Auth.this.getIntent().getExtras().getString("gid");
    Intent intent = new Intent(Auth.this, Plan.class);
    intent.putExtra("gname", gname);
    intent.putExtra("gid", gid);
    intent.putExtra("key", key);
    startActivity(intent);
...
else if(response.contains("none"))
{
    ...
    finish();
                ...
                e.printStackTrace();
                android.app.AlertDialog.Builder alertDialog = new android.app.AlertDialog.Builder(Auth.this);
                alertDialog.setTitle("Error!");
                alertDialog.setMessage("We have encountered an error while verifying your code!.");
                alertDialog.setPositiveButton("OK", new Auth.C02821());
                alertDialog.setNegativeButton("Cancel", new Auth.C02832());
